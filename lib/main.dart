    ...
    if (defaultTargetPlatform == TargetPlatform.android) {
      ...
      if (hasVibrator == true) {
        try {
          ...
          if (kDebugMode)
            print('🎛️ Amplitude control support: $hasAmplitudeControl');
...
Future<void> _manualVibration() async {
  try {
    if (kDebugMode)
      print('🔧 Attempting manual vibration via platform channel');
...
Future<void> testAllHapticMethods() async {
  ...
  try {
    bool? hasAmplitudeControl = await Vibration.hasAmplitudeControl();
    if (kDebugMode)
      print('🧪 TEST 2 - hasAmplitudeControl(): $hasAmplitudeControl');
  } catch (e) {
  ...
  try {
    ...
    if (kDebugMode)
      print('🧪 TEST 4 - Amplitude vibrate(100ms, 128) ERROR: $e');
  }
  ...
  try {
    ...
    if (kDebugMode)
      print('🧪 TEST 5 - HapticFeedback.mediumImpact() ERROR: $e');
  }
  ...
  try {
    await HapticFeedback.selectionClick();
    if (kDebugMode)
      print('🧪 TEST 8 - HapticFeedback.selectionClick(): SUCCESS');
    await Future.delayed(Duration(milliseconds: 200));
  } catch (e) {
    if (kDebugMode)
      print('🧪 TEST 8 - HapticFeedback.selectionClick() ERROR: $e');
  }
...
class TimerState {
  ...
  void dispose() {
    if (kDebugMode) {
      print('⚠️ Disposing TimerState: $id');
    }
  ...
  void updateFromFirestore(Map<String, dynamic> data) {
    ...
    if (!isRunning) {
      ...
      if (kDebugMode) {
        print('📝 Updated display value from Firebase: $timerValue (stopped)');
      }
    } else {
      if (kDebugMode) {
        print('⏭️ Skipped display value update (timer is running)');
      }
    }
...
class _TimerListState extends State<TimerList>
  ...
  void initState() {
    ...
    _initializeUserSession().then((_) async {
      ...
      if (wasInActiveSession) {
        ...
        if (kDebugMode) {
          print('🔄 Rejoining active session: $TIMER_SESSION_ID');
        }
        _isInActiveSession = true;

        // ⚠️ IMPORTANT: Backup current configuration before rejoining
    ...
    // Initialize custom distance controller
  ...
  Future<void> _initializeUserSession() async {
    try {
      ...
      if (kDebugMode) {
        ...
        print('🔗 Session ID: $TIMER_SESSION_ID');
      }
      ...
      if (kDebugMode) {
        print('✅ User session initialized and presence registered');
      }
  ...
  Future<void> _updateUserPresence() async {
    try {
      ...
      if (shouldSetOwner) {
        ...
        if (kDebugMode) {
          print('👑 Set session owner: $_currentUserId');
        }
      } else {
        ...
        if (_sessionOwnerId == null && !_isHost) {
          ...
          for (var doc in usersSnapshot.docs) {
            if (doc.id != _currentUserId) {
              ...
              if (kDebugMode) {
                print(
                    '👑 Inferred session owner (first user found): $_sessionOwnerId');
              }
              break;
            }
          }
        ...
        if (kDebugMode) {
          print(
              '👑 Session owner: $_sessionOwnerId (I am ${_isHost ? "HOST" : "GUEST"})');
        }
      }
    } catch (e) {
  ...
  Future<void> _removeUserPresence({bool isVoluntaryQuit = false}) async {
    try {
      ...
      if (kDebugMode) {
        print(
            '🚪 Removed user presence for $_currentUserId (voluntary: $isVoluntaryQuit)');
      }
    } catch (e) {
  ...
  Future<void> _fetchConnectedUsers() async {
    try {
      if (kDebugMode) {
        print(
            '🔍 Starting fetch for session: $TIMER_SESSION_ID, currentUser: $_currentUserId');
      }
      ...
      if (kDebugMode) {
        print('🔍 Fetched ${snapshot.docs.length} total documents');
      ...
      setState(() {
        ...
        if (kDebugMode) {
          print(
              '🔍 Fetched users: ${_connectedUsers.length + 1} total (Mode: ${_isOnlineMode ? "ONLINE" : "OFFLINE"})');
          print('📋 User IDs: ${_connectedUsers.keys.toList()}');
          if (wasOnline != _isOnlineMode) {
            print('🔄 Mode changed: $wasOnline → $_isOnlineMode');
          }
        }
      });
    } catch (e) {
  ...
  Future<void> _handleUserRemoval({bool shouldRestoreBackup = true}) async {
    if (kDebugMode) {
      print(
          '🚨 _handleUserRemoval called with shouldRestoreBackup: $shouldRestoreBackup');
    ...
    if (kDebugMode) {
      print('🆕 Generated new session code: $newSessionCode');
    }
    ...
    if (shouldRestoreBackup) {
      ...
      if (kDebugMode) {
        print('✅ After restore: ${runnerNames.length} timers');
      }
    } else {
      if (kDebugMode) {
        print('📌 Keeping current timers (no backup restore)');
      }
  ...
  void _listenToConnectedUsers() {
    ...
    _usersListener = firestore
      ...
      for (var change in snapshot.docChanges) {
        if (change.type == DocumentChangeType.removed) {
          ...
          if (_sessionOwnerId != null && removedUserId == _sessionOwnerId) {
            ...
            if (kDebugMode) {
              print(
                  '👑 Session owner removed: $removedUserId (voluntary: $wasVoluntaryQuit)');
            }

            // If owner voluntarily quit, kick all other users
            if (wasVoluntaryQuit && removedUserId != _currentUserId) {
              if (kDebugMode) {
                print('🚫 Session owner quit voluntarily - kicking all users');
          ...
          if (kDebugMode) {
            print('👋 User removed: $removedUserId');
          }
        }
      }
      ...
      if (!currentUserExists && !_userWasRemoved && !_isVoluntaryJoin) {
        ...
        if (kDebugMode) {
          print('🚫 You were removed from the session');
        }
      ...
      setState(() {
        ...
        if (kDebugMode) {
          if (oldUsers.length != _connectedUsers.length) {
            print(
                '👥 User count changed: ${oldUsers.length} → ${_connectedUsers.length}');
          }
          print(
              '👥 Connected users: ${_connectedUsers.length + 1} (Mode: ${_isOnlineMode ? "ONLINE" : "OFFLINE"})');
          print('👥 Current user IDs: ${_connectedUsers.keys.toList()}');
        }
      });

      if (kDebugMode) {
        print('🔄 setState completed in listener');
      }
    });
  }
  ...
  void _showRemoteActionIndicator(String userId,
    ...
    if (kDebugMode) {
      print('🔔 _showRemoteActionIndicator called:');
      print('   userId: $userId');
      print('   action: $action');
    ...
    if (!_isOnlineMode) {
      if (kDebugMode) {
        print('⚠️ Skipping indicator - not in online mode');
      }
      return;
    }
    ...
    if (kDebugMode) {
      print(
          '✅ Indicator state set: _lastActionUserId=$_lastActionUserId, _lastActionType=$_lastActionType, _lastActionTimerId=$_lastActionTimerId');
    }
  ...
  Future<void> _saveSessionConfiguration() async {
    try {
      ...
      if (kDebugMode) {
        print('💾 Session configuration saved: ${runnerNames.length} timers');
      }
    } catch (e) {
      if (kDebugMode) {
        print('❌ Error saving session configuration: $e');
      }
    }
  }
    ...
    try {
      ...
      if (doc.exists && doc.data() != null) {
        ...
        if (data['runnerNames'] != null) {
          ...
          if (kDebugMode) {
            print(
                '📥 Session configuration loaded: ${runnerNames.length} timers');
          }
        }
      }
    } catch (e) {
      if (kDebugMode) {
        print('❌ Error loading session configuration: $e');
      }
    }
  }
  ...
  void _backupCurrentConfiguration() {
    ...
    if (kDebugMode) {
      print(
          '💾 Backed up current configuration: ${runnerNames.length} timers, session: $_backupSessionId');
      print('💾 Backup timer names: ${_backupRunnerNames?.join(", ")}');
      print('💾 Backup state count: ${_backupTimerStates?.length}');
    }
  }
  ...
  void _restoreBackupConfiguration() {
    ...
    if (_backupRunnerNames != null && _backupTimerStates != null) {
      ...
      if (kDebugMode) {
        print('📥 Restored backup configuration: ${runnerNames.length} timers');
      ...
      if (kDebugMode) {
        print('⚠️ Cannot restore backup - backup data is null');
      }
    }
  }
  ...
  Future<void> _leaveSession() async {
    try {
      ...
      if (_isHost) {
        ...
        if (kDebugMode) {
          print('👑 Host is quitting - kicking all guests');
        }
        ...
        if (kDebugMode) {
          print('👤 Guest is quitting - restoring backup timers');
        }
        ...
        if (kDebugMode) {
          print('👋 Left session, new session: $TIMER_SESSION_ID');
        }
  ...
  Future<void> _joinSession(String sessionCode) async {
    try {
      ...
      if (kDebugMode) {
        print('🔄 Switching from session $TIMER_SESSION_ID to $sessionCode');
      ...
      if (kDebugMode) {
        print('🔗 Joined session: $TIMER_SESSION_ID');
      }
      ...
      if (kDebugMode) {
        print('✅ Presence updated in new session');
      }
      ...
      if (mounted) {
        setState(() {
          if (kDebugMode) {
            print(
                '🔄 UI refreshed - connected users: ${_connectedUsers.length}, online mode: $_isOnlineMode');
          }
        });
      }
  ...
  Future<void> _performInitialSync() async {
    try {
      ...
      if (kDebugMode) {
        print(
            '🔄 Initial sync: Found ${snapshot.docs.length} timers in Firebase');
        print('📋 Session initialized: ${sessionDoc.exists}');
      }
      ...
      if (!shouldLoadFromFirebase && snapshot.docs.isEmpty) {
        ...
        if (recheckTimers.docs.isNotEmpty ||
          ...
          if (kDebugMode) {
            print('📥 Another user initialized session - loading their data');
          }
        ...
        if (kDebugMode) {
          print(
              '📤 Initializing fresh session - syncing local timers to Firebase');
        }
        ...
        if (kDebugMode) {
          print(
              '📥 Loading existing timers from Firebase (${snapshot.docs.length} timers)');
        }
          ...
          if (shouldBeRunning) {
            ...
            if (data['lastLapStartTime'] != null) {
              try {
                ...
                if (kDebugMode) {
                  print(
                      '🕐 Lap running duration: ${lapRunningDuration.inMilliseconds}ms, total lap elapsed: ${remoteLapElapsed}ms');
                }
              } catch (e) {
            ...
            if (kDebugMode) {
              print('🟡🟡🟡 Creating periodic timer for UI updates 🟡🟡🟡');
            }
            ...
            if (kDebugMode) {
              print('✅ Started timer $timerId from Firebase state');
        ...
        if (extractedRunnerNames.isNotEmpty) {
          if (kDebugMode) {
            print(
                '📝 Updating runner names from Firebase: $extractedRunnerNames');
          }
          ...
          if (!wasInActiveSession) {
            ...
            if (kDebugMode) {
              print('💾 Saved runner names to SharedPreferences');
            }
          } else {
            if (kDebugMode) {
              print(
                  '⏭️ Skipped saving runner names (in active session, preserving backup)');
            }
          }
          ...
          if (kDebugMode) {
            print(
                '🔑 Regenerating all row keys for synced session (${runnerNames.length} runners)');
          }
          rowKeys = List.generate(
            runnerNames.length,
            (_) => GlobalKey<_TimerRowState>(),
          ...
          if (kDebugMode) {
            print('⏳ Waiting 50ms for periodic timers to start');
          }
        ...
        if (mounted) {
          if (kDebugMode) {
            print(
                '🔄 Triggering immediate UI update with initial timer values');
  ...
  void _initializeFirebaseListeners() {
    ...
    Future.delayed(Duration(milliseconds: 2000), () {
      ...
      if (kDebugMode) {
        print('✅ Skip window expired - forcing UI rebuild');
    ...
    if (kDebugMode) {
      print('🔥 Firebase listener initialized for session: $TIMER_SESSION_ID');
    }
    ...
    _timersListener = firestore
      ...
      if (_justCompletedInitialSync) {
        if (kDebugMode) {
          print('⏭️ Skipping listener update - just completed initial sync');
        }
        return;
      }
      ...
      if (kDebugMode) {
        print(
            '🔥 Received ${snapshot.docChanges.length} changes from Firebase');
      }
      ...
      for (var change in snapshot.docChanges) {
        if (change.type == DocumentChangeType.added ||
          ...
          if (data != null) {
            ...
            if (kDebugMode) {
              print(
                  '🔥 Update for timer: $timerId, isRunning: ${data['isRunning']}');
            }
            ...
            if (timerState != null) {
              ...
              if (kDebugMode) {
                print(
                    '🔍 Timer update: $timerId, actionUserId: $actionUserId, currentUserId: $_currentUserId');
              }

              if (actionUserId != null && actionUserId == _currentUserId) {
                // This is an echo of our own action - skip it
                if (kDebugMode) {
                  print('⏭️ Skipping own update for $timerId (userId match)');
                }
                continue;
              }
              ...
              if (actionUserId != null && actionUserId != _currentUserId) {
                if (kDebugMode) {
                  print('👤 Remote action from user: $actionUserId');
                  print(
              ...
              if (kDebugMode) {
                print('✅ Applying remote update to $timerId');
              }
              ...
              if (shouldBeRunning && !isCurrentlyRunning) {
                ...
                if (kDebugMode) {
                  print('▶️ Starting timer $timerId from remote');
                }
                ...
                if (kDebugMode) {
                  print('🕐 Server elapsed time: ${serverElapsed}ms');
                }
                ...
                if (kDebugMode) {
                  print('⏹️ Stopping timer $timerId from remote');
                }
                ...
                if (kDebugMode) {
                  print(
                      '✅ Timer $timerId stopped, final time: ${timerState.timerValue}');
                }
              } else if (shouldBeRunning && isCurrentlyRunning) {
                ...
                if (serverElapsed > localElapsed) {
                  if (kDebugMode) {
                    print(
                        '⏱️ Syncing to server time: $serverElapsed ms (local was $localElapsed ms)');
                  }
                  timerState.stopwatchOffset =
                      serverElapsed - timerState.stopwatch.elapsedMilliseconds;
                } else {
                  if (kDebugMode) {
                    print(
                        '⏭️ Keeping local time: $localElapsed ms (server was $serverElapsed ms)');
                  }
                }
                ...
                if ((remoteLapElapsed - localLapElapsed).abs() > 100) {
                  ...
                  if (kDebugMode) {
                    print(
                        '⏱️ Syncing lap time: $remoteLapElapsed ms (local was $localLapElapsed ms)');
                  }
                  timerState.lapStopwatchOffset = remoteLapElapsed -
                      timerState.lapStopwatch.elapsedMilliseconds;
                }
  ...
  Future<void> _syncToFirebase(TimerState timerState) async {
    if (timerState.isSyncing) {
      if (kDebugMode) {
        print('⏸️ Skipping sync for ${timerState.id} - already syncing');
      }
      return;
    }
    ...
    try {
      ...
      if (kDebugMode) {
        print(
            '📤 Syncing ${timerState.id} to Firebase: isRunning=${timerState.isRunning}, elapsed=${timerState.elapsedMilliseconds}ms, userId=$_currentUserId');
      }
      ...
      if (_currentUserId.isEmpty) {
        if (kDebugMode) {
          print('⚠️ Warning: _currentUserId is empty, using fallback');
        }
      ...
      if (kDebugMode) {
        print('✅ Sync complete for ${timerState.id}');
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  TimerState _getTimerState(int index) {
    ...
    if (!timerStates.containsKey(timerId)) {
      if (kDebugMode) {
        print('⚠️ Creating NEW timer state for $timerId (not found in map)');
  ...
  void _startTimer(TimerState timerState, {bool syncToFirebase = true}) async {
    if (kDebugMode) {
      print(
          '🎬 _startTimer called for ${timerState.id}, syncToFirebase: $syncToFirebase');
    }
      ...
      try {
        ...
        if (doc.exists) {
          ...
          if (data != null && (data['isRunning'] ?? false)) {
            ...
            if (kDebugMode) {
              print(
                  '🔄 Timer ${timerState.id} already running remotely, syncing instead of starting fresh');
            }
    ...
    if (syncToFirebase) {
      if (kDebugMode) {
        print('📤 Calling _syncToFirebase for ${timerState.id}');
      }
      _syncToFirebase(timerState);
    } else {
      if (kDebugMode) {
        print('⏭️ Skipping Firebase sync for ${timerState.id}');
      }
    }
  }
  ...
  void _stopTimer(TimerState timerState, {bool syncToFirebase = true}) {
    ...
    if (kDebugMode) {
      print('🛑 Stopping timer ${timerState.id} locally');
    }
  ...
  List<Widget> _buildDistanceResults() {
    final distances = [
      {'name': '${_customDistance}m', 'meters': _customDistance},
          ...
          children: [
            ...
            ...paceTypes.map((paceType) {
              return Expanded(
                child: Center(
                  child: Text(
                    paceType['label'] as String,
                    style: TextStyle(
                      fontSize: 11,
                      fontWeight: FontWeight.bold,
                      color: paceType['color'] as Color,
                    ),
                  ),
                ),
              );
    ...
    // Divider
    results.add(Divider(height: 1, thickness: 1, color: Colors.grey[300]));
    ...
    // Add definition section at the bottom
                ...
                child: RichText(
                  text: TextSpan(
                    ...
                    children: [
                      ...
                      TextSpan(text: def['desc'] as String),
                    ],
                  ),
                ),
              ),
  ...
  // New helper method for calculating time with a specific pace
  double _calculateTimeForDistanceWithPace(
      int distanceInMeters, double pacePerKm) {
    final pacePerKmInSeconds = pacePerKm * 60;
  ...
  double _calculateTimeForDistance(int distanceInMeters) {
    return _calculateTimeForDistanceWithPace(distanceInMeters, _paceValue);
  }
        ...
        children: [
          ...
          AnimatedBuilder(
            ...
            builder: (context, child) {
              // Full screen width for pace calculation panel
